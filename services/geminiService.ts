...
// Retries a function up to `maxRetries` times on 503 errors,
...
async function withRetry<T>(fn: () => Promise<T>, maxRetries = 3): Promise<T> {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.warn(`Gemini 503 — retrying in ${delay / 1000}s (attempt ${attempt + 1}/${maxRetries})`);
...
export const geminiService = {
  async getBookSummary(book: Book): Promise<string> {
    try {
      ...
      return "Could not generate AI summary at this time. Please try again in a moment.";
    }
  },
